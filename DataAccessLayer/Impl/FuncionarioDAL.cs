...
namespace DataAccessLayer.Impl
{
    public class FuncionarioDAL : IFuncionarioDAL
    {
        ...
        public async Task<SingleResponse<Funcionario>> GetInformationToVerify(int id)
        {
            try
            {
                return ResponseFactory<Funcionario>.CreateSuccessItemResponse(await _db.Funcionario.Include(f => f.Cargo).Select(f=> new Funcionario{ID = f.ID,IsFirstLogin = f.IsFirstLogin,Cargo = new Cargo { NivelPermissao = f.Cargo.NivelPermissao } }).FirstOrDefaultAsync(f=>f.ID== id));
            }
            catch (Exception ex)
            {
                return ResponseFactory<Funcionario>.CreateFailureItemResponse(ex);
